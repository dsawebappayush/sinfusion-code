...
def train_image_diffusion(cfg):
    ...
    tb_logger = pl.loggers.TensorBoardLogger("/kaggle/working/sinfusion/lightning_logs/", name=cfg.image_name, version=cfg.run_name)
...
def train_video_predictor(cfg):
    ...
    tb_logger = pl.loggers.TensorBoardLogger("/kaggle/working/sinfusion/lightning_logs/", name=cfg.image_name, version=cfg.run_name + '_predictor')
    trainer = pl.Trainer(max_steps=training_steps,
                         accelerator="auto", 
...
def train_video_projector(cfg):
    ...
    tb_logger = pl.loggers.TensorBoardLogger("/kaggle/working/sinfusion/lightning_logs/", name=cfg.image_name, version=cfg.run_name + '_projector')
    trainer = pl.Trainer(max_steps=training_steps,
                         accelerator="auto", 
                         logger=tb_logger, log_every_n_steps=10,
                         callbacks=model_callbacks)
...
def train_video_interpolator(cfg):
    ...
    tb_logger = pl.loggers.TensorBoardLogger("/kaggle/working/sinfusion/lightning_logs/", name=cfg.image_name, version=cfg.run_name + '_interpolator')
    trainer = pl.Trainer(max_steps=training_steps,
                         accelerator="auto", 
                         logger=tb_logger, log_every_n_steps=10,
                         callbacks=model_callbacks)
